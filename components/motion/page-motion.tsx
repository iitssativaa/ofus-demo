...
export function PageMotion({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const startEntry = () => {
      ...
      const shellDuration = panels.length > 1 ? BOX_DURATION : BOX_PHASE_DURATION;
      const rise = (36 * document.documentElement.clientWidth) / 1980;
      const frames: Keyframe[] = [
        { opacity: 0, transform: `translate3d(0,${rise}px,0)` },
        { opacity: 1, transform: "translate3d(0,0,0)" },
      ];
      const enter = (target: HTMLElement, options: KeyframeAnimationOptions) => {
        const animation = target.animate(frames, { easing: ENTRY_EASING, fill: "backwards", ...options });
        entryAnimations.push(animation);
        animations.push(animation);
      };

      if (header) enter(header, { duration: shellDuration });
      panels.forEach((panel, index) => {
        enter(panel, { duration: shellDuration, delay: Math.round(index * stagger) });
        // Direct children are the panel content phase. Restricting the query to
        // :scope prevents nested panels and their descendants from animating twice.
        [...panel.querySelectorAll<HTMLElement>(":scope > *")]
          .filter((content) => !content.matches('[hidden], [aria-hidden="true"]'))
          .forEach((content) => enter(content, { duration: CONTENT_DURATION, delay: BOX_PHASE_DURATION }));
      });

      node.dataset.motionState = "entering";
      if (!entryAnimations.length) { completeEntry(); return; }
    ...
    const run = async () => {
      ...
      } catch { /* Motion never blocks authentication. */ }
    ...
    const prepare = (event: Event) => {
      ...
      const dialog = link.closest<HTMLElement>('[aria-modal="true"]');
      if (dialog && !dialog.closest('[hidden], [inert], [aria-hidden="true"]') && dialog.getClientRects().length > 0) return;
    ...
    const click = (event: MouseEvent) => {
      ...
      void fade.finished.catch(() => undefined).then(() => { if (!cancelled) router.push(href); });
    };
  ...
  return <div ref={root} key={pathname} className="ofus-page" data-motion-state="preparing">{children}</div>;
}
