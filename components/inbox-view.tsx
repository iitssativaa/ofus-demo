...
const inboxStateKey = "ofus.inbox-view.v1";
type InboxState = { tab: "new" | "activity"; search: string; companyFilter: string; personFilter: string; fromDate: string; toDate: string };
const emptyInboxState: InboxState = { tab: "new", search: "", companyFilter: "all", personFilter: "all", fromDate: "", toDate: "" };
const safeDate = (value: unknown) => typeof value === "string" && /^\d{4}-\d{2}-\d{2}$/.test(value) && !Number.isNaN(Date.parse(`${value}T12:00:00`)) ? value : "";
const safeFilter = (value: unknown) => typeof value === "string" && value.length <= 100 ? value : "all";
const readInboxState = (): InboxState => {
  if (typeof window === "undefined") return emptyInboxState;
  try {
    const value = JSON.parse(sessionStorage.getItem(inboxStateKey) ?? "null") as Partial<InboxState> | null;
    if (!value || typeof value !== "object") return emptyInboxState;
    return { tab: value.tab === "activity" ? "activity" : "new", search: typeof value.search === "string" ? value.search.slice(0, 200) : "", companyFilter: safeFilter(value.companyFilter), personFilter: safeFilter(value.personFilter), fromDate: safeDate(value.fromDate), toDate: safeDate(value.toDate) };
  } catch { return emptyInboxState; }
};
const writeInboxState = (value: InboxState) => { try { sessionStorage.setItem(inboxStateKey, JSON.stringify(value)); } catch { /* Oturum depolaması kullanılamıyorsa görünüm çalışmaya devam eder. */ } };
...
export function InboxView(props: { workspaceActivities: WorkspaceActivity[] }) {
  const ready = useSyncExternalStore(subscribeReady, () => true, () => false);
  return ready ? <InboxContents {...props} /> : <section className="panel p-6" aria-busy="true">Gelen kutusu yükleniyor…</section>;
}
function InboxContents({ workspaceActivities }: { workspaceActivities: WorkspaceActivity[] }) {
  const { tasks, companies, projects, users, taskError } = useWorkspace();
  const [initial] = useState(readInboxState);
  const [tab, setTab] = useState<"new" | "activity">(initial.tab);
  const [search, setSearch] = useState(initial.search);
  const [companySelection, setCompanyFilter] = useState(initial.companyFilter);
  const [personSelection, setPersonFilter] = useState(initial.personFilter);
  const [fromDate, setFromDate] = useState(initial.fromDate);
  const [toDate, setToDate] = useState(initial.toDate);
  const companyFilter = companySelection === "all" || companies.some((item) => item.id === companySelection) ? companySelection : "all";
  const personFilter = personSelection === "all" || users.some((item) => item.id === personSelection) ? personSelection : "all";
  useEffect(() => { writeInboxState({ tab, search, companyFilter, personFilter, fromDate, toDate }); }, [tab, search, companyFilter, personFilter, fromDate, toDate]);
