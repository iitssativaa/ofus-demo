...
export function ResourceLinksSection({ ownerType, ownerId, title = "Kaynaklar", readOnly = false, editHref }: { ownerType: ResourceOwnerType; ownerId: string; title?: string; readOnly?: boolean; editHref?: string }) {
  ...
  const remove = async () => {
    ...
    setSaving(true);
    try { await deleteResourceLink(deleting.id); setResources((current) => current.filter((item) => item.id !== deleting.id)); setDeleting(null); }
    catch { setError("Kaynak silinemedi. Lütfen tekrar deneyin."); }
    finally { deleteLock.current = false; setSaving(false); }
  };

  return <section className="panel overflow-hidden"><div className="panel-header"><div><h2 className="section-title">{title}</h2><p className="section-subtitle">{loading ? "Yükleniyor…" : `${resources.length} bağlantı`}</p></div>{readOnly ? editHref ? <Link href={editHref} className="text-link">Projede aç</Link> : null : <button type="button" onClick={() => setEditing(null)} className="secondary-button"><Plus size={14} />Kaynak Ekle</button>}</div>{error ? <p role="alert" className="border-b border-rose-100 bg-rose-50 px-4 py-2 text-xs font-semibold text-rose-700">{error}</p> : null}<div className="divide-y divide-slate-100">{!loading && !resources.length ? <p className="p-5 text-sm text-slate-400">Henüz bağlantı eklenmedi.</p> : resources.map((resource) => { const Icon = typeIcons[resource.type]; return <article key={resource.id} className="flex min-w-0 items-center gap-3 px-4 py-3"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-slate-50 text-slate-500"><Icon size={16} /></span><div className="min-w-0 flex-1"><div className="flex min-w-0 items-center gap-2"><p className="truncate text-sm font-semibold text-slate-800">{resource.title}</p><span className="shrink-0 text-[10px] font-bold text-slate-400">{resourceTypeLabels[resource.type]}</span></div><a href={resource.url} target="_blank" rel="noopener noreferrer" className="mt-0.5 block truncate text-xs text-indigo-600 hover:text-indigo-800" title={resource.url}>{resource.url}</a>{resource.note ? <p className="mt-1 truncate text-[11px] text-slate-400">{resource.note}</p> : null}</div><a href={resource.url} target="_blank" rel="noopener noreferrer" className="icon-button shrink-0" aria-label={`${resource.title} bağlantısını aç`}><ExternalLink size={15} /></a>{!readOnly ? <><button type="button" onClick={() => setEditing(resource)} className="icon-button shrink-0" aria-label={`${resource.title} kaynağını düzenle`}><Pencil size={15} /></button><button type="button" onClick={() => setDeleting(resource)} className="icon-button shrink-0 text-rose-600" aria-label={`${resource.title} kaynağını sil`}><Trash2 size={15} /></button></> : null}</article>; })}</div>
