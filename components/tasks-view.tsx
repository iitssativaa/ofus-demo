...
type TaskViewPrefs = { section?: "all" | "active" | "completed" | "cancelled"; search?: string; assignee?: string; project?: string; company?: string; status?: string; priority?: string; archiveFrom?: string; archiveTo?: string; sort?: string };
const allowedSections = new Set(["all", "active", "completed", "cancelled"]);
const allowedSorts = new Set(["manual", "due", "priority", "newest"]);
const allowedStatuses = new Set(["all", "To Do", "In Progress", "Waiting", "Review", "Done"]);
const allowedPriorities = new Set(["all", "Low", "Medium", "High", "Urgent"]);
const safeChoice = (value: unknown, allowed?: Set<string>) => typeof value === "string" && value.length <= 100 && (!allowed || allowed.has(value)) ? value : "all";
const safeDate = (value: unknown) => typeof value === "string" && /^\d{4}-\d{2}-\d{2}$/.test(value) && !Number.isNaN(Date.parse(`${value}T12:00:00`)) ? value : "";
const readTaskViewPrefs = (): TaskViewPrefs => { try { const value: unknown = JSON.parse(sessionStorage.getItem("ofus.task-filters.v1") ?? "null"); if (!value || typeof value !== "object" || Array.isArray(value)) return {}; const item = value as Record<string, unknown>; return { section: safeChoice(item.section, allowedSections) as TaskViewPrefs["section"], search: typeof item.search === "string" ? item.search.slice(0, 200) : "", assignee: safeChoice(item.assignee), project: safeChoice(item.project), company: safeChoice(item.company), status: safeChoice(item.status, allowedStatuses), priority: safeChoice(item.priority, allowedPriorities), archiveFrom: safeDate(item.archiveFrom), archiveTo: safeDate(item.archiveTo), sort: safeChoice(item.sort, allowedSorts) }; } catch { return {}; } };
...
export function TasksView(props: { loadError?: string }) {
  const ready = useSyncExternalStore(subscribeReady, () => true, () => false);
  return ready ? <TasksContents {...props} /> : <section className="panel p-6" aria-busy="true">Görevler yükleniyor…</section>;
}

function TasksContents({ loadError = "" }: { loadError?: string }) {
  ...
  const [initialPrefs] = useState(readTaskViewPrefs);
  const [section, setSection] = useState<"all" | "active" | "completed" | "cancelled">(() => requestedScope && allowedSections.has(requestedScope) ? requestedScope as "all" | "active" | "completed" | "cancelled" : initialPrefs.section ?? "all");
  const [view, setView] = useState<"list" | "board">(() => { const requested = searchParams.get("view"); if (requested === "list" || requested === "board") return requested; try { return localStorage.getItem("ofus.task-view.v1") === "list" ? "list" : "board"; } catch { return "board"; } });
  const [search, setSearch] = useState(initialPrefs.search ?? "");
  const deferredSearch = useDeferredValue(search.toLowerCase());
  const [assigneeSelection, setAssignee] = useState(initialPrefs.assignee ?? "all");
  const [projectSelection, setProject] = useState(initialPrefs.project ?? "all");
  const [companySelection, setCompany] = useState(initialPrefs.company ?? "all");
  const [statusSelection, setStatus] = useState(initialPrefs.status ?? "all");
  const [prioritySelection, setPriority] = useState(initialPrefs.priority ?? "all");
  const [archiveFrom, setArchiveFrom] = useState(initialPrefs.archiveFrom ?? "");
  const [archiveTo, setArchiveTo] = useState(initialPrefs.archiveTo ?? "");
  const [sort, setSort] = useState(initialPrefs.sort ?? "manual");
  const [filtersOpen, setFiltersOpen] = useState(false);
  const assignee = assigneeSelection === "all" || users.some((item) => item.id === assigneeSelection) ? assigneeSelection : "all";
  const project = projectSelection === "all" || projects.some((item) => item.id === projectSelection) ? projectSelection : "all";
  const company = companySelection === "all" || companies.some((item) => item.id === companySelection) ? companySelection : "all";
  const status = allowedStatuses.has(statusSelection) ? statusSelection : "all";
  const priority = allowedPriorities.has(prioritySelection) ? prioritySelection : "all";
  ...
  const filterCount = commonFilterCount + (section === "active" || section === "all"
  ...
  useEffect(() => { try { localStorage.setItem("ofus.task-view.v1", view); } catch { /* Görünüm depolaması kullanılamıyorsa görevler çalışmaya devam eder. */ } }, [view]);
  useEffect(() => { try { sessionStorage.setItem("ofus.task-filters.v1", JSON.stringify({ section, search, assignee, project, company, status, priority, archiveFrom, archiveTo, sort } satisfies TaskViewPrefs)); } catch { /* Oturum depolaması kullanılamıyorsa filtreler çalışmaya devam eder. */ } }, [section, search, assignee, project, company, status, priority, archiveFrom, archiveTo, sort]);
  const filtered = useMemo(() => tasks.filter((task) => {
    const inSection = section === "all" ? !isCancelled(task) : section === "active" ? activeStatuses.includes(task.status) && !isCancelled(task) : section === "completed" ? isFormallyCompleted(task) : isCancelled(task);
    ...
    return inSection && (!deferredSearch || haystack.includes(deferredSearch)) && (assignee === "all" || task.assigneeId === assignee) && (project === "all" || task.projectId === project) && (company === "all" || task.companyId === company) && (section !== "active" && section !== "all" || status === "all" || task.status === status) && (section === "cancelled" || priority === "all" || task.priority === priority) && (section === "active" || section === "all" || ((!archiveFrom || Boolean(archiveDate && archiveDate >= archiveFrom)) && (!archiveTo || Boolean(archiveDate && archiveDate <= archiveTo))));
  }).sort((a, b) => {
  ...
  const visibleIds = filtered.filter((task) => activeStatuses.includes(task.status) && !isCancelled(task)).map((task) => task.id);
  ...
  const filterControls = (mobile = false) => <div className={mobile ? "grid gap-3" : "task-filter-fields"}>
    ...
    {section === "active" || section === "all" ? <ThemedSelect className={mobile ? "input mt-0" : "filter-select"} ariaLabel="Duruma göre filtrele" value={status} onValueChange={setStatus} options={[{ value: "all", label: "Tüm durumlar" }, ...activeStatuses.map((item) => ({ value: item, label: statusLabels[item] })), ...(section === "all" ? [{ value: "Done", label: statusLabels.Done }] : [])]} /> : null}
  ...
  return <>
    ...
    <div className="task-sections mb-4 grid w-full grid-cols-4 rounded-xl border border-slate-200 bg-white p-1 shadow-sm sm:inline-flex sm:w-auto"><button onClick={() => setSection("all")} className={`view-button h-9 justify-center px-1 sm:px-2.5 ${section === "all" ? "view-button-active bg-slate-100" : ""}`}><Columns3 size={14} /><span className="inline">Tümü</span></button><button onClick={() => setSection("active")} className={`view-button h-9 justify-center px-1 sm:px-2.5 ${section === "active" ? "view-button-active bg-slate-100" : ""}`}><List size={14} /><span className="inline">Aktif</span></button><button onClick={() => setSection("completed")} className={`view-button h-9 justify-center px-1 sm:px-2.5 ${section === "completed" ? "view-button-active bg-slate-100" : ""}`}><History size={14} /><span className="inline">Tamamlanan</span></button><button onClick={() => setSection("cancelled")} className={`view-button h-9 justify-center px-1 sm:px-2.5 ${section === "cancelled" ? "view-button-active bg-slate-100" : ""}`}><Ban size={14} /><span className="inline">İptal Edilen</span></button></div>
      ...
      {section === "active" || section === "all" ? <>
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-3 py-2.5"><div className="flex min-w-0 flex-wrap items-center gap-2">{view === "list" ? <><Checkbox checked={allVisibleSelected} indeterminate={!allVisibleSelected && someVisibleSelected} disabled={!visibleIds.length || taskSaving} onChange={toggleAllVisible} ariaLabel="Görünen görevlerin tümünü seç" /><span className="whitespace-nowrap text-xs font-semibold text-slate-600">Tümünü Seç</span></> : null}<div className="ml-2 inline-flex rounded-lg bg-slate-100 p-1"><button onClick={() => setView("list")} className={`view-button ${view === "list" ? "view-button-active" : ""}`}><List size={14} />Liste</button><button onClick={() => { setSection("all"); setSort("manual"); setView("board"); }} className={`view-button ${view === "board" ? "view-button-active" : ""}`}><Columns3 size={14} />Pano</button></div></div><div className="ml-auto flex shrink-0 items-center gap-2 text-xs font-medium text-slate-500"><span className="hidden sm:inline">Sırala</span><ThemedSelect className="h-8 min-w-28 rounded-lg border border-transparent bg-transparent px-2 text-xs font-semibold text-slate-700 outline-none" ariaLabel="Görevleri sırala" value={sort} onValueChange={setSort} options={[{ value: "manual", label: "Manuel" }, { value: "due", label: "Son tarih" }, { value: "priority", label: "Öncelik" }, { value: "newest", label: "En yeni" }]} /></div></div>
        ...
        {view === "board" ? <TaskBoard tasks={filtered} manual={sort === "manual"} emptyMessage={hasFilters ? "Bu filtrelerle eşleşen görev bulunamadı." : "Henüz görev yok."} /> : <div className={`space-y-2 bg-slate-50/50 p-3 lg:hidden ${selectedIds.size ? "pb-24" : ""}`}>{filtered.length ? filtered.map((task) => <TaskCard key={task.id} task={task} showCountdown selectable={task.status !== "Done"} selected={selectedIds.has(task.id)} onSelectionChange={(checked) => setSelected(task.id, checked)} />) : <div className="p-8 text-center text-sm text-slate-400">{hasFilters ? "Bu filtrelerle eşleşen görev bulunamadı." : "Henüz görev yok."}</div>}</div>}
        {view === "list" ? <div className="hidden lg:block"><div className="overflow-x-auto"><div className="min-w-[930px]"><div className="ofus-task-table-head grid grid-cols-[32px_26px_minmax(180px,1.6fr)_minmax(140px,1fr)_64px_92px_150px_42px] gap-3 border-b border-slate-100 bg-slate-50/70 px-3 py-2 text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400"><span/><span/><span>Görev</span><span>Durum</span><span>Sorumlu</span><span>Öncelik</span><span>Son Tarih</span><span>Boyut</span></div>{filtered.length ? (sort === "due" ? ["Geciken", "Bugün", "Yaklaşan", "Tarihsiz"].map((label) => { const items = filtered.filter((task) => (taskIsOverdue(task) ? "Geciken" : taskDate(task) === localDate() ? "Bugün" : taskDate(task) ? "Yaklaşan" : "Tarihsiz") === label); return items.length ? <div key={label}><div className="ofus-task-group">{label} <span>{items.length}</span></div>{items.map((task) => <TaskRow key={task.id} task={task} showCountdown selectable={task.status !== "Done"} selected={selectedIds.has(task.id)} onSelectionChange={(checked) => setSelected(task.id, checked)} />)}</div> : null; }) : filtered.map((task) => <TaskRow key={task.id} task={task} showCountdown selectable={task.status !== "Done"} selected={selectedIds.has(task.id)} onSelectionChange={(checked) => setSelected(task.id, checked)} />)) : <div className="p-12 text-center text-sm text-slate-400">{hasFilters ? "Bu filtrelerle eşleşen görev bulunamadı." : "Henüz görev yok."}</div>}</div></div></div> : null}
