...
export function TasksView({ loadError = "" }: { loadError?: string }) {
  ...
  const filterControls = (mobile = false) => <div className={mobile ? "grid gap-3" : "flex flex-wrap items-center gap-2"}>
  ...
  return <>
    <PageHeader eyebrow="İş kuyruğu" title="Görevler" description={section === "active" ? `${filtered.length} aktif görev · sırala ve işi ilerlet.` : section === "completed" ? `${filtered.length} tamamlanan görev · başarılı kapanışları incele.` : `${filtered.length} iptal edilen görev · düşen işleri ve nedenlerini incele.`} actions={<button onClick={() => setQuickAddOpen(true)} className="primary-button"><Plus size={15} />Görev Ekle</button>} />
    ...
    <div className="mb-4 grid w-full grid-cols-3 rounded-xl border border-slate-200 bg-white p-1 shadow-sm sm:inline-flex sm:w-auto"><button onClick={() => setSection("active")} className={`view-button h-9 justify-center px-1 sm:px-2.5 ${section === "active" ? "view-button-active bg-slate-100" : ""}`}><List size={14} /><span className="hidden min-[390px]:inline">Aktif</span></button><button onClick={() => setSection("completed")} className={`view-button h-9 justify-center px-1 sm:px-2.5 ${section === "completed" ? "view-button-active bg-slate-100" : ""}`}><History size={14} /><span className="hidden min-[390px]:inline">Tamamlanan</span></button><button onClick={() => setSection("cancelled")} className={`view-button h-9 justify-center px-1 sm:px-2.5 ${section === "cancelled" ? "view-button-active bg-slate-100" : ""}`}><Ban size={14} /><span className="hidden min-[390px]:inline">İptal Edilen</span></button></div>
    <section className="panel overflow-hidden">
      <div className="flex gap-2 border-b border-slate-200 p-3 lg:items-center">
