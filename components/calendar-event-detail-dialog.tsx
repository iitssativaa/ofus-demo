...
export function CalendarEventDetailDialog({ event, users, timeZone, deleting, confirmDelete, error, onClose, onEdit, onDelete, onCancelDelete }: {
  ...
  const endDateLabel = event.endsAt && localDate(event.endsAt, timeZone) !== localDate(event.startsAt, timeZone)
    ? new Intl.DateTimeFormat("tr-TR", { weekday: "long", day: "numeric", month: "long", year: "numeric", timeZone }).format(new Date(event.endsAt))
    : null;
