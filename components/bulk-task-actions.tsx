...
function BulkActionDialog({ action, selectedCount, users, saving, onCancel, onApply }: {
  ...
  return <div className="responsive-dialog z-[80]" role="dialog" aria-modal="true" aria-labelledby="bulk-action-title">
    ...
    <form onSubmit={submit} className="responsive-dialog-panel max-w-md">
      ...
      <footer className="responsive-dialog-footer"><button type="button" disabled={saving} onClick={close} className="secondary-button">İptal</button><button type="submit" disabled={saving} className={action === "cancel" ? "inline-flex h-9 items-center justify-center gap-2 rounded-lg bg-rose-600 px-3.5 text-xs font-semibold text-white hover:bg-rose-700 disabled:opacity-60" : "primary-button disabled:opacity-60"}>{saving ? "Uygulanıyor…" : action === "cancel" ? "Görevleri İptal Et" : "Uygula"}</button></footer>
    </form>
  </div>;
}
