...
export function companyFromRow(row: CompanyRow): Company {
  ...
  return { id: row.id, name: row.name, color: colors[hash % colors.length], contactName: row.contact_name ?? "", phone: row.phone ?? "", email: row.email ?? "", website: row.website ?? "", notes: row.notes ?? "" };
}
...
export function projectFromRow(row: ProjectRow): Project {
  return { id: row.id, name: row.name, companyId: row.company_id, status: statusFromDatabase[row.status], progress: 0, memberIds: [], description: row.description, notes: row.notes ?? "" };
}
